...
def response_ranking_warmup_training(args,
...
def main():
    ...
    if args.do_train:
        ...
        global_step_fe, \
        re_loss, \
        fe_loss = response_ranking_warmup_training(args, train_intent_dataset, train_answer_intent_dataset,
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        checkpoints = [os.path.join(eval_output_dir, 'checkpoint-answer-' + str(idx)) for
                       idx in range(1300, 35000, 100)]
        if args.eval_all_checkpoints:
            checkpoints = list(
    ...
    if args.do_predict2 and args.local_rank in [-1, 0]:
        ...
        checkpoints = [os.path.join(eval_output_dir, 'checkpoint-answer_intent-' + str(idx)) for
                       idx in range(7000, 30000, 100)]
        if args.eval_all_checkpoints:
            checkpoints = list(
                os.path.dirname(c) for c in
                sorted(glob.glob(args.model_name_or_path + "/**/" + WEIGHTS_NAME, recursive=True))
