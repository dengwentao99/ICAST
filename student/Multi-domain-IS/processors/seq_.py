...
def collate_fn(batch):
    ...
    # max_len = max(all_sample_input_len_reward).item()
    # all_sample_input_ids_reward = all_sample_input_ids_reward[:, :max_len]
    # all_sample_input_mask_reward = all_sample_input_mask_reward[:, :max_len]
    # all_sample_segment_ids_reward = all_sample_segment_ids_reward[:, :max_len]
    # all_sample_feedback_idx_ids_reward = all_sample_feedback_idx_ids_reward[:, :max_len]
    # all_sample_label_is_answer = all_sample_label_is_answer[:, :max_len]
    # all_sample_label = all_sample_label[:, :max_len]
...
class WarmupProcessor(DataProcessor):
    ...
    def get_inference_examples(self, data_dir, path, data_mask=None):
        return self._create_examples_inference(self._read_text(os.path.join(data_dir, path)), "unlabeled", data_mask)

    def get_pseudo_examples(self, data_dir, path):
        return self._create_examples_pseudo(self._read_text(os.path.join(data_dir, path)), 'u')
    ...
    def _create_examples_pseudo(self, lines, set_type):
        examples = []
        for idx, line in enumerate(lines):
            ...
            unlabeled_data_mask = line['data_mask']
            pseudo_label = line['pseudo_label']
            ...
            answer_context = ' '.join(answer_context)
            for step in range(10):
                if unlabeled_data_mask[step] in [0, 1]:
                    continue
                if step == 0:
                    examples.append(InputExample(guid=guid,
                                                 label=1 if pseudo_label[step] > 0.5 else 0,
                                                 text_contexts=answer_context,
                                                 text_candidate=response,
                                                 text_feedback=feedback,
                                                 mode='answer1'))
            ...
            answer_context2 = ' '.join(answer_context2)
            for step in range(10):
                if unlabeled_data_mask[step] in [0]:
                    continue
                if step == 0:
                    examples.append(InputExample(guid=guid,
                                                 label=1 if pseudo_label[step] > 0.5 else 0,
                    ...
                    examples.append(InputExample(guid=guid,
                                                 ...
                                                 text_feedback=feedback,
                                                 mode='answer2'))
            if 2 not in unlabeled_data_mask:
                continue
            tags = ['OQ', 'FD', 'FQ', 'IR', 'PA', 'PF', 'NF', 'GG', 'O']
            intent_context = []
            for m in range(0, len(context), 2):
    ...
    def _create_examples_inference(self, lines, set_type, data_mask=None):
        """Creates examples for the training and dev sets."""
        examples = []
        if data_mask is not None:
            print(len(data_mask), len(lines))
            assert len(data_mask) == len(lines)
        for i, line in enumerate(lines):
            if data_mask is not None and data_mask[i] == 0:
                continue
            guid = "{}".format(i)
